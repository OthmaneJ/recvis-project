...
class TransformerDecoderMultiChannel(Decoder):
    """
    A transformer decoder with N masked layers.
    Decoder layers are masked so that an attention head cannot see the future.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        :param kwargs:
        """
        super(TransformerDecoderMultiChannel, self).__init__()
    ...
    def forward(
        ...
        **kwargs
        ...
        for layer in self.layers:
            x_1 = layer(x=x, memory=encoder_output[0], src_mask=src_mask, trg_mask=trg_mask)
            x_2 = layer(x=x, memory=encoder_output[1], src_mask=src_mask, trg_mask=trg_mask)
            x_3 = layer(x=x, memory=encoder_output[2], src_mask=src_mask, trg_mask=trg_mask)
