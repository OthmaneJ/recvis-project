...
from signjoey.encoders import Encoder, RecurrentEncoder, TransformerEncoder, TransformerEncoderMultiChannel
from signjoey.decoders import Decoder, RecurrentDecoder, TransformerDecoder, TransformerDecoderMultiChannel
...
class SignModelMultiChannel(nn.Module):
    """
    Base Model class
    """
    ...
    def forward(
        ...
        sgn: Tensor,
        sgn_mask: Tensor,
        sgn_body_mask: Tensor,
        sgn_face_mask: Tensor,
        sgn_face_dope: Tensor,
        sgn_body_dope: Tensor,
        sgn_lengths: Tensor,
        txt_input: Tensor,
        ...
        encoder_output1, encoder_output2, encoder_output3, encoder_hidden = self.encode(
            sgn=sgn, sgn_mask=sgn_mask, 
            sgn_face_mask=sgn_face_mask,
            sgn_body_mask=sgn_body_mask,
        ...
        if self.do_recognition:
            # Gloss Recognition Part
            # N x T x C
            gloss_scores = self.gloss_output_layer(encoder_output1)
            # N x T x C
            gloss_probabilities = gloss_scores.log_softmax(2)
            # Turn it into T x N x C
            gloss_probabilities = gloss_probabilities.permute(1, 0, 2)
    ...
    def encode(
        self, sgn: Tensor, sgn_mask: Tensor, sgn_face_mask: Tensor, sgn_body_mask: Tensor,
         sgn_length: Tensor,
        sgn_face_dope: Tensor, sgn_body_dope: Tensor
    ) -> (Tensor, Tensor):
        """
        ...
        # print(type(sgn_face_dope))
        # print(sgn_face_dope)
        src1, src2, src3 = self.sgn_embed(x1=sgn, x2=sgn_face_dope, x3=sgn_body_dope, mask=sgn_mask)
        return self.encoder(
            src1, src2, src3,
            src_length=sgn_length,
            mask=sgn_mask,
            face_mask=sgn_face_mask,
            body_mask=sgn_body_mask,
        )
    ...
    def get_loss_for_batch(
        ...
        decoder_outputs, gloss_probabilities = self.forward(
            sgn=batch.sgn,
            sgn_mask=batch.sgn_mask,
            sgn_face_mask=batch.sgn_face_mask,
            sgn_body_mask=batch.sgn_body_mask,
            sgn_face_dope=batch.sgn_face_dope,
            sgn_body_dope=batch.sgn_body_dope,
            sgn_lengths=batch.sgn_lengths,
            txt_input=batch.txt_input,
    ...
    def run_batch(
        ...
        encoder_output1, encoder_output2, encoder_output3, encoder_hidden = self.encode(
            sgn=batch.sgn, sgn_mask=batch.sgn_mask, sgn_face_mask=None, sgn_body_mask=None,
            sgn_face_dope=batch.sgn_face_dope, sgn_body_dope=batch.sgn_body_dope,
            sgn_length=batch.sgn_lengths
        )
...
def build_model_multi_channel(
    ...
    if cfg["encoder"].get("type", "recurrent") == "transformer":
        ...
        encoder = TransformerEncoderMultiChannel(
            **cfg["encoder"],
            emb_size=sgn_embed.embedding_dim,
            emb_dropout=enc_emb_dropout,
        )
    ...
    if do_translation:
        ...
        dec_emb_dropout = cfg["decoder"]["embeddings"].get("dropout", dec_dropout)
        if cfg["decoder"].get("type", "recurrent") == "transformer":
            decoder = TransformerDecoderMultiChannel(
                **cfg["decoder"],
                encoder=encoder,
                vocab_size=len(txt_vocab),
                emb_size=txt_embed.embedding_dim,
    ...
    model: SignModel = SignModelMultiChannel(
        encoder=encoder,
        gloss_output_layer=gloss_output_layer,
        decoder=decoder,
        sgn_embed=sgn_embed,
